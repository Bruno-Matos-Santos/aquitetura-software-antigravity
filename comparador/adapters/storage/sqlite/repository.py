...
class SqliteProductRepository(ProductRepository):
    ...
    def get_daily_lowest_price_history(
        self, product_id: UUID
    ) -> list[dict]:
        """Returns the daily lowest price for a product across all active listings."""
        sql = """
        ...
        with self._conn() as conn:
            for r in conn.execute(sql, (str(product_id),)).fetchall():
                # To match Chart.js format expectation: {x: 'YYYY-MM-DD', y: min_price}
                out.append({"x": r["date"], "y": r["min_price"]})
        return out
